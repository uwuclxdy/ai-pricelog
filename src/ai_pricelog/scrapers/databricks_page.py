...
bill input only). the matched row's cells are strict: an unknown
rate-cell shape raises (plan #22), never reads as unpriced. rows the
match scan passes over (odd cell counts, unmapped names) are additive
drift detection already reported. the page carries no context/max-tokens
column and no peak tier, so those fields stay unset. mode is chat.
...
the page has no dbu table, or the matched row's rate cells are outside
the known shapes.
"""
...
def scrape(cfg: ProviderCfg, model_id: str) -> Pricing | None:
    ...
    for row in table.find("tbody").find_all("tr", recursive=False):
        ...
        if input_rate is None:
            return None  # the row is provisioned-only for this model
