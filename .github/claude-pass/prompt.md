...
- google: the gemini-api changelog carries intro-price windows, deprecations, and free-tier notes; fetch with `?hl=en` pinned (a locale-less fetch returned Japanese from one vantage). the developers.googleblog feed spans all dev products (label feeds 404) and re-publishes daily with a new build date; the date rides the hashed prose, so the channel reports a change every run with zero content delta. OBSERVED 2026-08-29 (fix: docs/todo.md #18).
