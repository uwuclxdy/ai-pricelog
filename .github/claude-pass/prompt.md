...
- `currency` (default `USD`) + `unit` (default `tokens`) name the source quote; `currency_rate` + `currency_rate_date` are the conversion provenance; the mtok price fields always hold USD. all four optional; the two rate fields never join the diff. `effective_at` (optional) is the date the rate or schedule becomes effective, when a source announces one ahead of time; absent = valid at observation. consumers clamp rows to `effective_at` <= the query date.
...
- openrouter rows add `name` and keep only unmapped pricing keys verbatim under `extra`. alias entries and dated-canonical snapshots get no row. the typed mtok fields are: `input_mtok` / `output_mtok` / `cache_read_mtok` / `cache_write_mtok` / `cache_write_1h_mtok` plus the modality keys `image_mtok`, `audio_mtok`, `input_audio_cache_mtok`, `internal_reasoning_mtok`, `image_output_mtok`, `audio_output_mtok`; `web_search_usd` is a per-REQUEST fee, never per-token. scheduled overrides land under `window_rates` (one entry per override: optional `days` = lowercase weekday names, absent = every day; optional `window` = `[start_hhmm, end_hhmm]`, absent = whole day; per-rate mtok keys present only when the override carries them, absent keys inherit the base price; an entry may carry `quota_multiplier` instead of or beside rate keys, and consumers never price from a multiplier). `timezone` rides beside `window_rates` (deepseek Asia/Shanghai, openrouter UTC, zai Asia/Singapore). volume overrides (`min_prompt_tokens`) land under `volume_rates` (one entry per override: `min_tokens` + per-rate mtok keys, the window inherit rule). deepseek peak rows map onto window_rates entries: the base fields are the off-peak rates, one entry per peak window carries the peak rates plus the weekday day-set. rows stored before schema v3 keep modality keys and volume overrides under `extra` (no history rewrite).
- a removal row carries `{"removed": true}` plus the final price snapshot (the last priced row's comparable fields); it means the source stopped listing the model. accept it as valid.
- a zero price is a price: `0.0` rate fields are real rows, negative amounts read as unpriced.
- `data/index.json` regenerates each run: latest row per `(source, model_id)` plus `first_seen`. a removed model keeps its last prices and gains `removed_at`. the top-level `version` mirrors `data/schema.json`.
- `data/schema.json` names the row-format version; a new or renamed top-level row key bumps it together with the validation key set.
