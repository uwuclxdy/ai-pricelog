...
# dispatch a duplicate run; that duplicate opens no PRs (the pipeline's
# pending scan) and costs one quiet watchdog pass, while the 300s headroom is
# what keeps a seconds-late systemd timer fire from suppressing the next one
set -euo pipefail
jq -er '
