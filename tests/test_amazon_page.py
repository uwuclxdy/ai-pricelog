...
# api's per-1K usd; the row round-trip must land them x1000 per-1M
RATES = {
...
def test_detect_skips_nova_names_without_a_model_card(
    monkeypatch: pytest.MonkeyPatch, caplog: pytest.LogCaptureFixture
):
    # nova 2.0 lite carries a chat on-demand row in the fixture: a nova name
    # with no docs model card is additive drift, skipped with a warning
    # (plan #22). nova canvas sits in the fixture too, but only as a T2I
    # image product: no chat axis, so the name never reaches the join and
    # stays silently invisible (no crash, no guessed id)
    feed(monkeypatch)
    with caplog.at_level(logging.WARNING):
        assert detector.detect(cfg()) == EXPECTED_IDS
    assert "Nova 2.0 Lite" in caplog.text
...
def test_detect_nova_names_without_a_carded_join_raises(monkeypatch: pytest.MonkeyPatch):
    # a fully stale name map (every nova name outside it) is structural, not
    # drift; the message names the names it saw
    ...
    with pytest.raises(FetchError, match="Nova 2.0 Lite"):
        detector.detect(cfg())
