...
1. for each draft PR the run opened: diff `origin/mommy...<branch>` against the PR branch.
...
- databricks: the foundation-model-serving page (restructured 2026-09-05) serves two per-token tables statically, "Standard Pay Per Token (DBU Per 1M Tokens)" and "Priority Pay Per Token", each with a Model + tier-span header row and an Input | Output | Cache read sub-header ("Kimi K3 42.857 / 214.286 / 4.286"); the per-hour tables (Provisioned Throughput, Batch Inference) never match the pin. ids = an explicit display-name dict per table, derived from the stored openrouter id set (vendor prefix stripped; priority-table rows -> "-priority" ids; the U+2316 regional-uplift marker strips); the merged "GLM-5.2, 5.3" row covers both store ids at one rate pair. a priced row with an unmapped name skips with a warning (plan #22), so one new model can no longer blind the provider. the matched row's cells stay strict (an unknown rate-cell shape raises), rows the scan passes over are tolerated; a tier-span reword raises rather than mis-classifying. "-" output = embedding rows bill input only, stored with output 0.0; "-" input = unpriced row, skipped. dedup: the page's undated deepseek-v4-pro/deepseek-v4-flash spellings -> the dated store spellings deepseek-v4-pro-0813/deepseek-v4-flash-0731. the DBU->USD rate is 0.07 (the pricing-page AI card, client-rendered; cited via google's index + third-party consensus; DBU rates differ per cloud, the announce watches /aws/ so re-probe before trusting another cloud). announce: docs release-notes index (sub-page links client-rendered).
...
- moonshot: the only official changelog (`platform-changelog.md`) is stale (newest entry 2025-04-07); post-2025 announcements have no docs-side dated surface. `llms.txt` + the sitemap enumerate the whole docs tree. the 2026-09-11 llms.txt consolidation merged the per-model pricing pages into `pricing/chat.md` (one DocTable carrying every current model; the old `chat-*.md` urls now serve the merged page or the models list); kimi-k2.5 and the moonshot-v1 series were retired 2026-08-31 (models.md warning + deprecated table), so their removal rows verify against models.md's deprecated section.
