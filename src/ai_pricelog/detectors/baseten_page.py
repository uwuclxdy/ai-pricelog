...
"Popular models" links sit outside the table. a page without the Model
APIs table or a row without a library link is a parse failure (FetchError).
"""
...
def detect(cfg: ProviderCfg) -> list[str]:
    ...
    return list(dict.fromkeys(_model_id(row[0], cfg.detector_url) for row in rows))
...
def _model_rows(soup: BeautifulSoup, url: str) -> list[list[Tag]]:
    ...
    for grid in soup.select("div.grid"):
        ...
        if [cell.get_text(" ", strip=True) for cell in cells] != _HEADER_CELLS:
            continue
        rows = [
